...
function createListView(query_type, parent_url, folder_array, file_array) {
	...
	if (query_type == 0 || query_type == 1) {
		...
		html += '<div id="btnParent" class="ui-btn ui-btn-text ui-btn-parent" data-icon="home" data-role="button" data-iconpos="left" ';
	...
	for (var i = 0; i < folder_array.length; i++) {
		...
		html += '" data-icon="carat-r">';

		//html += "<input type='checkbox' name='checkbox-0' />";

		html += "<a href='#' data-transition='slide' class='ui-btn ui-btn-icon-right ui-icon-carat-r'>";
		...
		if (query_type == 2) {
			if (folder_array[i].type == "usbdisk")
				html += "<span id='icon' style='filter: invert(1);' class='sicon usbdisk ui-li-thumb ";
			else if (folder_array[i].type == "sambapc") {
				if (folder_array[i].online == 1)
					html += "<span id='icon' style='filter: invert(1);' class='sicon sambapc ui-li-thumb ";
				else
					html += "<span id='icon' style='filter: invert(1);' class='sicon sambapcoff ui-li-thumb ";
			}
		} else {
			html += "<span id='icon' class='sicon folder ui-li-thumb ";
		}
		...
		html += "<h2>" + folder_array[i].shortname;
    ...
    */
	...
	g_timer_filelist = setInterval(function () {
		...
		html += '<div style="display: flex;align-items: center;flex-direction: row;">';

		html += '<div class="ui-file-icon-div">';
...
function createLayout() {
	...
	$("#btn-home").text(m.getString('btn_homepage'));
	$("#btn-refresh").text(m.getString('btn_refresh'));
	$("#btn-logout").text(m.getString('title_logout'));
	$("#btn-setting").text(m.getString('title_setting'));
	$("#btn-delete").text(m.getString('btn_delselect'));
	$("#btn-rename").text(m.getString('btn_rename'));
	$("#btn-share").text(m.getString('btn_sharelink'));
	$("#btn-createfolder").text(m.getString('btn_newdir'));
...
(function ($) {
	...
	$(document).on("click", "#btnParent", function () {
	...
	$(document).on("click", "a#btn-home", function () {
		doPROPFIND("/", false, function () {
	...
	$(document).on("click", "a#btn-share", function () {
	...
	$(document).on("click", "a#btn-rename", function () {
	...
	$(document).on("click", "a#btn-createfolder", function () {
	...
	$(document).on("click", "#select-choice-custom-button", function () {
		//alert("click");
