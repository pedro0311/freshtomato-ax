...
*** Variables ***
${SUITE_SETUP_LOG}     ==== NewOB Auto test Start! ====
${SUITE_DOWN_LOG}     ==== NewOB Auto test End! ====
${USERNAME_1}       admin
...
NEWRE_setup
    ...
    ELSE
        ...
        IF    ${flag}==1
            Log To Console    sw_mode is repeater mode, cannot do auto newob
            FAIL
        END
    END
...
SET_NEWOB_AUTO
    ...
    ENABLE_NEWOB_AUTO
    ${cfg_auto_newob_duration}    Write_Read    Telnet    0    nvram get cfg_auto_newob_duration    ${DUT_END}
    ${cfg_auto_newob_remaining}    Write_Read    Telnet    0    nvram get cfg_auto_newob_remaining    ${DUT_END}

NEWOB_AUTO
    Write_wait    Telnet    cat /tmp/newob_onboarding.json\r\n    180s    1s    ${NEWEW_MAC}
    Write_wait    Telnet    cat /jffs/.sys/cfg_mnt/cfg_dbg.log\r\n    180s    1s    aimesh_newob_auto enable, trigger ${NEWEW_MAC} newob
    sleep    10
...
ENABLE_NEWOB_AUTO
    ...
    Write_Read    Telnet    0    nvram set aimesh_newob_auto=1    ${DUT_END}
    Write_Read    Telnet    0    nvram set cfg_auto_newob_duration=${NEWOB_AUTO_TIME}    ${DUT_END}
    ${data}    Create Dictionary    action_mode=apply    rc_service=restart_cfgsync
    WebApply    ${USERNAME_2}    ${PASSWD_2}    ${DUT_PATH_2}    applyapp.cgi    ${data}

DISABLE_NEWOB_AUTO
    Telnet.Switch Connection    1st conn
    Write_Read    Telnet    0    nvram unset aimesh_newob_auto    ${DUT_END}
    Write_Read    Telnet    0    nvram unset cfg_auto_newob_duration    ${DUT_END}
    Write_Read    Telnet    0    nvram unset cfg_auto_newob_remaining    ${DUT_END}
