...
with open("router/rom/rom/tmpfs.json") as jsonFile:
	jsonObject = json.load(jsonFile)
	jsonFile.close()

tmpfsArray = jsonObject['tmpfs']
#  print("+"+"-"*62+"+")
for model in tmpfsArray:
...
print("Require TMPFS: %6.2f MB" % float(float(tmpfsRequireSize)/1048576))
...
with open("router/tmpfs_requirement.txt", 'w') as out_file:
	out_file.write("%d" % tmpfsRequireSize)
